...
// ─── Pre-registration email OTP (keyed by email, no account yet) ─────────────

const REG_OTP_TTL      = 600; // 10 minutes
const REG_OTP_MAX_TRIES = 3;

function regOtpKey(email: string) { return `reg:otp:${email.toLowerCase()}`; }

async function storeRegOtp(email: string, otp: string): Promise<void> {
  await redis.set(regOtpKey(email), JSON.stringify({ otp, attempts: 0 }), 'EX', REG_OTP_TTL);
}

async function verifyRegOtp(email: string, inputOtp: string): Promise<'ok' | 'invalid' | 'expired' | 'locked'> {
  const raw = await redis.get(regOtpKey(email));
  if (!raw) return 'expired';

  const record = JSON.parse(raw) as { otp: string; attempts: number };
  if (record.attempts >= REG_OTP_MAX_TRIES) {
    await redis.del(regOtpKey(email));
    return 'locked';
  }
  if (record.otp !== inputOtp) {
    const ttl = await redis.ttl(regOtpKey(email));
    await redis.set(regOtpKey(email), JSON.stringify({ ...record, attempts: record.attempts + 1 }), 'EX', ttl > 0 ? ttl : REG_OTP_TTL);
    return 'invalid';
  }
  await redis.del(regOtpKey(email));
  return 'ok';
...
export const AuthService = {
  ...
  async sendRegistrationOtp(email: string): Promise<{ maskedEmail: string }> {
    const existing = await prisma.user.findUnique({ where: { email: email.toLowerCase() } });
    if (existing) throw Object.assign(new Error('Email already in use'), { status: 409 });

    const otp = generateOtp();
    await storeRegOtp(email, otp);
    sendOtpEmail(email, '', otp).catch(() => {});

    return { maskedEmail: maskEmail(email) };
  },

  /** Step 2: verify OTP then create account and return tokens */
  async register(server: FastifyInstance, data: RegisterInput & { otp: string }) {
    const result = await verifyRegOtp(data.email, data.otp);
    if (result === 'expired') throw Object.assign(new Error('Code expired, request a new one'), { status: 410 });
    ...
    // Double-check email not taken between OTP send and now
  ...
  /** Resend OTP for registration — email not yet in DB */
  async resendRegistrationOtp(email: string): Promise<{ maskedEmail: string }> {
    const existing = await prisma.user.findUnique({ where: { email: email.toLowerCase() } });
    if (existing) throw Object.assign(new Error('Email already in use'), { status: 409 });
